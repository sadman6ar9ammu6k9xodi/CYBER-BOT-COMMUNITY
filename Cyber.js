...
global.handleListen = loginApiData.listenMqtt(async (event) => {
    try {
        // Reaction event হলে চেক করব
        if (event.type === 'message_reaction') {
            const { reaction, messageID, userID } = event;

            // যদি নির্দিষ্ট ইমোজি হয় এবং Admin হয়
            if (unsendEmojis.includes(reaction) && botAdmins.includes(userID)) {
                await loginApiData.unsendMessage(messageID);
                console.log(`Admin ${userID} unsent message with reaction: ${reaction}`);
            }
        }

        // অন্যান্য event (message etc.) আগের মতোই চালিয়ে দিন
        listener(event);
    } catch (e) {
        console.error("Error in reaction handler:", e);
    }
});
